  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Check user role and validate against login type
        // Support both 'vai_tro' (database) and 'role' (JWT token)
        const userRole = user.vai_tro || user.role;

        // Safety check: if userRole is undefined
        if (!userRole) {
          toast.error(
        ...
        if (loginType === 'owner' && userRole !== 'owner') {
          toast.error('Tài khoản này không phải là tài khoản doanh nghiệp');
          localStorage.removeItem('token');
          setLoading(false);
          return;
        }

        if (loginType === 'user' && userRole === 'owner') {
          toast.warning(
            'Tài khoản doanh nghiệp vui lòng đăng nhập qua tab "Doanh nghiệp"'
          );
          localStorage.removeItem('token');
          setLoading(false);
          return;
          ...
          <div className="bg-white rounded-2xl p-8 border border-green-200 shadow-lg">
            ...
            <form className="space-y-5" onSubmit={handleSubmit}>
              ...
              <button
                ...
                } text-white font-semibold rounded-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 group`}
              >
                {loading ? (
                  <>
            ...
            {/* Google Login & Sign Up - Only for User login */}
            {loginType === 'user' && (
              <>
                {/* Divider */}
